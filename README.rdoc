...
* 1.4.0   * Added warnings in case of enumeration field not matched (status, tracker, priority)
          ...
          * Added support for importing spent times on imported issues, and uses an Activity field and a User_spent_time field if provided. Otherwise, sets the spent time to the default activity, and for the user the issue is assigned to
